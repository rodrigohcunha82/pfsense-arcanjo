...
Version 1.2.1 - 2024-01-12

Written by Rodrigo Henriques<rodrigo.cunha@mexti.com.br>
*/
...
//
     ...
     // interfaces_discovery_infos
     function pfz_mexti_interface_discovery(){
          $ifdescrs = get_configured_interface_with_descr(true);
          $ifaces=array();
          $json_string = '{"data":[';
          foreach ($ifdescrs as $ifdescr => $ifname){	     
               ...
               $ifaces[$ifname] = $ifinfo;
               $json_string .= '{';
               $json_string .= '"{#IFNAME}":"' . $ifname . '"';
               $json_string .= ',"{#HWIF}":"' . $ifinfo['hwif'] . '"';
               $json_string .= ',"{#ENABLE}":"' . $ifinfo['enable'] . '"';
               $json_string .= ',"{#STATUS}":"' . $ifinfo['status'] . '"';
               $json_string .= ',"{#IPADDR}":"' . $ifinfo['ipaddr'] . '"';
               $json_string .= ',"{#MACADDR}":"' . $ifinfo['macaddr'] . '"';
               $json_string .= '},';
          }
          $json_string = rtrim($json_string,",");
          $json_string .= "]}";
     ...
     // interfaces_values_infos
     function pfz_mexti_interface_value($interface){
          $ifdescrs = get_configured_interface_with_descr(true);
          foreach ($ifdescrs as $ifdescr => $ifname){
               if ($ifname == $interface){
                    $ifinfo = get_interface_info($ifdescr);
...
function pfz_discovery($section){
     switch (strtolower($section)){     
          ...
          case "temperature_sensors":
               ...
               break;
		  case "mexti_interface_discovery":
               pfz_mexti_interface_discovery();
               break;
     }         
}
...
switch (strtolower($argv[1])){     
     ...
     case "temperature":
          ...
          break;
	 case "mexti_interface";
		  pfz_mexti_interface_value($argv[2]);
		  break;
     default:
          pfz_test();
}